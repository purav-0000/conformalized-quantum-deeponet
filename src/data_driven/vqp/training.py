...
class TrainingRunner:
    ...
    def _train_one_instance(self, model_dir: Path, seed: int):
        ...
        with torch.no_grad():
            ...
            for i, index in enumerate(worst_indices):
                plt.figure(figsize=(10, 6))
                plt.plot(x_test[1], y_test[index], 'r-', label=f'Ground Truth (Error: {errors[index]:.2%})')
                plt.plot(x_test[1], y_pred[index], 'b--', label='Prediction')
            ...
            for i, index in enumerate(best_indices):
                plt.figure(figsize=(10, 6))
                plt.plot(x_test[1], y_test[index], 'g-', label=f'Ground Truth (Error: {errors[index]:.2%})')
                plt.plot(x_test[1], y_pred[index], 'b--', label='Prediction')
        ...
        with torch.no_grad():
            for i in range(5):
                ...
                plt.figure()
                plt.plot(np.linspace(0, 10, len(x_test[0][index])), x_test[0][index])
                plt.show()

                plt.figure()
                plt.plot(x_test[1][:, 0], y_test[index], color='orange')
                plt.plot(x_test[1][:, 0], model.predict(x_test)[index], color='green')
                # plt.savefig(f"{i}.png")
                plt.show()
    ...
    def _setup_dde_model(self, x_train: Tuple, y_train: np.ndarray, x_test: Tuple, y_test: np.ndarray) -> dde.Model:
        ...
        for i in range(3):
            ...
            plt.figure()
            plt.plot(np.linspace(0, 10, len(x_train[0][index])), x_train[0][index])
            plt.show()

            plt.figure()
            plt.plot(x_train[1][:, 0], y_train[index], color='orange')
            # plt.savefig(f"{i}.png")
            plt.show()
